...
class SplitMix64:
    """Cross-language generator used only to make byte-identical benchmark inputs."""
    def __init__(self,seed):self.state=seed
    def next(self):
        self.state=(self.state+0x9e3779b97f4a7c15)&MASK;z=self.state
        z=((z^(z>>30))*0xbf58476d1ce4e5b9)&MASK
        z=((z^(z>>27))*0x94d049bb133111eb)&MASK
        return (z^(z>>31))&MASK
    def value(self):return np.float32(((self.next()>>40)/float(1<<24))*2-1)
...
def synthetic_cases():
    ...
    for axis,values in axes.items():
        for axis_value in values:
            p=base.copy();p[axis]=axis_value
            x=rng.normal(size=(p["rows"],p["dimensions"])).astype(np.float32)
            if p["density"]<1:
                x[rng.random(x.shape)>p["density"]]=0;x=sparse.csr_matrix(x)
            yield f"synthetic/{axis}/{axis_value}",x,p

def matched_input(p):
    rng=SplitMix64(SEED);rows=p["rows"];columns=p["dimensions"]
    if p["density"]==1:
        return np.asarray([rng.value() for _ in range(rows*columns)],dtype="<f4").reshape(rows,columns)
    values=[];indices=[];indptr=[0];threshold=int(p["density"]*(1<<24))
    for _ in range(rows):
        for column in range(columns):
            value=rng.value()
            if rng.next()>>40>=threshold:continue
            values.append(value);indices.append(column)
        indptr.append(len(values))
    return sparse.csr_matrix((np.asarray(values,dtype="<f4"),np.asarray(indices,dtype=np.int64),np.asarray(indptr,dtype=np.int64)),shape=(rows,columns))

def matched_cases():
    base=dict(rows=256,dimensions=16,neighbors=15,components=2,density=1.0,metric="euclidean")
    axes={"rows":[64,1024],"dimensions":[2,128],"neighbors":[5,50],"components":[2,8],"density":[.05,.5],"metric":["euclidean","cosine"]}
    for axis,values in axes.items():
        for axis_value in values:
            p=base.copy();p[axis]=axis_value
            yield f"synthetic/{axis}/{axis_value}",matched_input(p),p
    p=dict(rows=1000,dimensions=5000,neighbors=15,components=2,density=.01,metric="cosine")
    yield "synthetic/sparse-text",matched_input(p),p
...
def fit_once(x,p):
    ...
    umap.UMAP(n_neighbors=p["neighbors"],n_components=p["components"],metric=p["metric"],n_epochs=100,init=init,random_state=SEED,n_jobs=1).fit(x)
...
def run_once(x,p):
    ...
    start=time.perf_counter_ns();fit_once(x,p);timings["fit_end_to_end"]=time.perf_counter_ns()-start
    timings["total"]=timings["fit_end_to_end"];return timings

def main():
    ap=argparse.ArgumentParser();ap.add_argument("--suite",choices=("fast","full","matched"),default="fast");ap.add_argument("--case",action="append",help="run only this exact case name; repeatable");ap.add_argument("--repeats",type=int,default=5);ap.add_argument("--warmups",type=int,default=1);ap.add_argument("--output",type=Path);ap.add_argument("--prepare-public",action="store_true");ap.add_argument("--cache",type=Path,default=Path("benchmarks/.data"));args=ap.parse_args()
    ...
    cases=list(matched_cases()) if args.suite=="matched" else list(synthetic_cases())+list(public_cases(args.suite,args.cache))
    for name,x,p in cases:
        ...
        gc.collect();tracemalloc.start();start=time.perf_counter_ns();fit_once(x,p);allocation_timings={"fit_end_to_end":time.perf_counter_ns()-start};_,allocated_peak=tracemalloc.get_traced_memory();gc.collect();retained,_=tracemalloc.get_traced_memory();tracemalloc.stop()
